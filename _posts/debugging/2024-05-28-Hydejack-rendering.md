...
---
부푼 기대감을 안고 Github 블로그를 운영하기 시작했으나.. 선택한 Hydejack 테마에서 수학 수식 렌더링 문제가 발생했다 ㅠㅠ

그러나, 이 문제를 해결한 과정을 기록한 블로그도 없어서 해결하기 매우 쉽지 않았다

기술 블로그에서 수식을 포기하는 건 있을 수가 없고, 다른 테마로 교체할 까도 수만번 고민했으나.. 
...
katex는 속도가 빠르지만, 지원 기능이 적다.
반면, mathjax는 지원 기능이 더 많은데, 속도가 느리다.

Hydejack은 기본적으로 katex에 맞춰져 있지만, 현 시점에서 업데이트가 제대로 되어 있지 않아 수식 렌더링이 제대로 되지 못하는 문제가 발생한다.
...
물론, mathjax를 사용해도 문제가 바로 해결되진 않는다. config 파일에서 math_engine에 mathjax를 넣어도 렌더링은 안된다..

결국, GPT의 힘을 빌려 다음의 자바스크립트 코드를 HTML 파일에 삽입했다.
...
HTML 코드를 살펴본 결과, katex와 mathjax가 모두 적용돼서 수식이 중복되는 것으로 확인되었다.

본인은 katex와 관련된 모든 파일을 하나하나 뜯어봐서 모두 주석 처리하였다.

여기서 파일들을 모두 언급하기는 어렵지만.. 본인은 katex가 적용되지 않도록 VS Code에서 katex를 검색해 관련된 코드를 모두 주석 처리해 주었다.

안정적인 블로그 관리를 위해 로컬에서 테스트하면서 실행하길 권한다.
...
그러나, 페이지에 첫 접속하면 수식 렌더링이 되지 않았고, 이상하게도 새로고침 후 수식이 제대로 표현되었다.

앞서 GPT가 주었던 MathJax를 렌더링하는 코드를 post.html에 삽입해주었다. 

본인은 블로그 내 모든 페이지에서 수식 렌더링이 적용되도록 위 코드를 head.html에 포함하였다.

![image](https://github.com/minsuk1003/minsuk1003.github.io/assets/63490319/8ea94f3c-cf38-41a0-8d98-2de9ec208f09)

드디어 수식 렌더링 문제를 해결하는 데 성공했다.
