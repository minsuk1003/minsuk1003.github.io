...
> 그러나, Numpy를 통해 구현되는 정규 방정식은 $O(n^{2.4})$에서 $O(n^3)$ 사이의 매우 높은 복잡도를 가진다.
{: .prompt-warning}
...
Scikit-learn의 LinearRegression 클래스는 scipy.linalg.lstsq()를 기반으로 하는데, 이 함수는 $X$의 유사역행렬 $X^+$을 통해 $\hat\theta = X^+y$를 계산한다.
...
이를 통해 정규 방정식보다 훨씬 효율적으로 $\hat\theta$를 계산할 수 있고,

기존 정규 방정식은 $X^T X$의 역행렬이 없다면 작동하지 않지만, 유사역행렬은 항상 구할 수 있다는 장점이 있다.
...
$$
\frac{\partial }{\partial \theta_j} R(\theta) = -\frac{2}{n} X^T (y - X\theta)
$$
...
- `p-value`: $P > |t| = 0.000$는 각 계수가 통계적으로 유의미함을 나타냄 $(P < 0.05)$
