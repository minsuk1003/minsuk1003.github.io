...
tags: ml_dl
description: >
  Ensemble (앙상블), Voting (투표), Bagging (배깅), Pasting (페이스팅), Random Forest (랜덤 포레스트)
...
m개의 샘플에서 랜덤으로 하나를 추출할 때, 선택되지 않을 확률은 $(1 - \frac{1}{m})$이고, m번 반복하면 선택되지 않을 확률은 $$(1 - \frac{1}{m})^m$$이 된다. m을 무한대로 늘리면, 이 값은 로피탈의 정리에 의해 $$(e^{-1} = 0.3679)$$와 같아지게 된다. 따라서, 샘플이 선택될 확률은 1 - 0.3679 = 0.6321로 약 63.2%이다. $$
