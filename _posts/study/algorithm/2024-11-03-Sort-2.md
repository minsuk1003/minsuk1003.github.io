...
categories:
  ...
  - algorithm
description: >
  Quick Sort (퀵 정렬), Merge Sort (병합 정렬), Heap Sort (힙 정렬), Tim Sort (팀 정렬), Stable, In-place
---
...
## 목차
<!-- TOC -->
  * [1. Quick Sort (퀵 정렬)](#1-quick-sort-퀵-정렬)
  * [2. Merge Sort (병합 정렬)](#2-merge-sort-병합-정렬)
  * [3. Heap Sort (힙 정렬)](#3-heap-sort-힙-정렬)
  * [4. Tim Sort (팀 정렬)](#4-tim-sort-팀-정렬)
  * [정리](#정리)
  * [참조](#참조)
<!-- TOC -->
...
![quick_sort](https://github.com/user-attachments/assets/9daaefe5-9446-4090-8ec2-a2654f15461c)

퀵 정렬은 재귀적으로 리스트를 나누는 과정에서 평균적으로 $O(\log n)$ 단계로 분할되며, 각 단계마다 최대 $n$개의 연산이 필요하다.
...
**Stable**: O (같은 값의 순서가 유지됨)
- 분할 과정에서 `left`, `middle`, `right` 리스트에 추가될 때 순서가 유지된다.

**In-place**: X (별도 메모리 공간이 필요함)
- 새로운 리스트를 생성하므로, 별도 메모리 공간이 필요하다.
...
**Stable**: X (같은 값의 순서가 바뀔 수 있음)
- 분할 과정에서 데이터 위치를 교환하기 때문에, 같은 값을 가진 데이터 순서가 유지되지 않는다.

**In-place**: O (별도 메모리 공간이 필요 없음)
- 배열 내에서 직접 정렬을 수행하며, 추가 메모리가 필요하지 않다. 
...
![merge_sort](https://github.com/user-attachments/assets/4c755998-96c6-4a18-8489-cbf81a105d90)

**시간 복잡도**: 모든 경우 $O(n \log n)$
- 병합 정렬은 리스트를 반으로 나눠 재귀적으로 정렬하고, 최종적으로 $n$개의 원소를 합병하는 과정을 반복한다.

**Stable**: O
- 병합 과정에서 원래 데이터의 순서를 유지하며 병합하므로, 같은 값을 가진 데이터 순서는 유지된다.

**In-place**: X
- 그러나, 병합 과정에서 추가 배열이 필요하므로, 추가 메모리 공간이 요구된다.
...
![heap_sort](https://github.com/user-attachments/assets/79123328-b3b4-409f-b4d5-be151c6a260c)

**시간 복잡도**: 모든 경우 $O(n \log n)$

**Stable**: X
- 힙 정렬은 힙을 재구성하면서 데이터의 순서를 고려하지 않는다.

**In-place**: O
- 힙을 배열 내에서 정렬하므로 추가 메모리는 필요하지 않다.
...
1. [Sorting Algorithm Explained](http://www.xybernetics.com/techtalk/SortingAlgorithmsExplained/SortingAlgorithmsExplained.html)
2. [신찬수 교수 유튜브](https://youtu.be/Du-EHAUE0kM?si=-d_ud1gNag7QLY20)